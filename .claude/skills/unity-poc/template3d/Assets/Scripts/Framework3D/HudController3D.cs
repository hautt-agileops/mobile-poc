...
namespace Fighter3D
{
    // uGUI HUD built entirely in code: health + meter bars, round pips, timer, banner. Identical
    // pattern to the 2D HUD — uGUI is dimension-agnostic; only the controls text differs.
    public class HudController3D : MonoBehaviour
    {
        Image hpFillL, hpFillR, mpFillL, mpFillR, installL, installR;
        Text nameL, nameR, timerTxt, banner, controls;
        Image[] pipImgsL, pipImgsR;
        Font font;
        GameObject canvasGO;
        ...
        public void Build(CharacterDef3D a, CharacterDef3D b, int bestOf)
        {
            ...
            nameL = Label(root, a.displayName.ToUpper(), new Vector2(0.04f, 0.80f), TextAnchor.UpperLeft, 22, a.bodyColor);
            nameR = Label(root, b.displayName.ToUpper(), new Vector2(0.96f, 0.80f), TextAnchor.UpperRight, 22, b.bodyColor);
            installL = InstallDot(root, true, a.installColor);
            installR = InstallDot(root, false, b.installColor);

            timerTxt = Label(root, "60", new Vector2(0.5f, 0.92f), TextAnchor.MiddleCenter, 40, Color.white);
            banner = Label(root, "", new Vector2(0.5f, 0.55f), TextAnchor.MiddleCenter, 64, Color.white);
            controls = Label(root, ControlsText(), new Vector2(0.5f, 0.06f), TextAnchor.MiddleCenter, 15, new Color(1, 1, 1, 0.55f));
        ...
        string ControlsText() =>
            "P1  move WASD  jump Space  block LShift   F light  G heavy  H special/super  T install\n"
            + "P2  move ↑←↓→  jump RCtrl  block RShift   Num1 light  Num2 heavy  Num3 special/super  Num5 install";

        void MakeBar(Transform root, bool left, float y, Color fill, out Image bg, out Image fillImg, float width = 0.42f, float height = 26)
        {
        ...
        public void Tick(Fighter3D a, Fighter3D b, float timeLeft, int winsL, int winsR)
        {
        ...
        public void ShowControls(bool on) => controls.enabled = on;
    }
}
